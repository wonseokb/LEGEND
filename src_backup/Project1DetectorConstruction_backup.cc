...
void Project1DetectorConstruction::setMaterial1Property()//member set-function1 Property
{
  ...
  G4double refractionIndex = 1.58;
  std::vector<G4double> refractionIndices(emissionEnergies.size(), refractionIndex);//***

  Material1_Mpt->AddProperty("RINDEX", &emissionEnergies[0], &refractionIndices[0], (G4int) emissionEnergies.size());//3
  Material1_Mpt->AddConstProperty("WLSTIMECONSTANT", 8.5 * ns);//catch!//5 (1.0) 
  Material1_Mpt->AddProperty("WLSABSLENGTH", &wlsAbsorptionEnergies[0], &wlsAbsorptions[0], (G4int) wlsAbsorptionEnergies.size());//1
  Material1_Mpt->AddProperty("WLSCOMPONENT", &wlsEmissionEnergies[0], &wlsEmissions[0], (G4int) wlsEmissionEnergies.size());//2
...
void Project1DetectorConstruction::setMaterial2Property()//Okay
{
  ...
  Material2_Mpt->AddProperty("WLSABSLENGTH", &wlsAbsorptionEnergies[0],  &wlsAbsorptions[0],  (G4int) wlsAbsorptionEnergies.size());//1
  Material2_Mpt->AddProperty("WLSCOMPONENT", &wlsEmissionEnergies[0], &wlsEmissions[0], (G4int) wlsEmissionEnergies.size());//2
  Material2_Mpt->AddConstProperty("WLSTIMECONSTANT", 0.01 * ns);//5
  Material2_Mpt->AddProperty("RINDEX", &refractionIndexEnergies[0], &refractionIndices[0], (G4int) refractionIndexEnergies.size());//3
  Material2->SetMaterialPropertiesTable(Material2_Mpt);//6
}
...
G4VPhysicalVolume* Project1DetectorConstruction::makeMode2()//
{
  ...
  */
  ...
  G4double EJ280Radius = 2.54*1.5*cm;
  //-----------------------------------------------------------------------------------------------------------------------------------------  
...
G4LogicalVolume* Project1DetectorConstruction::makePmtLogicalVolume()//function22 makeVolume 3
{
  ...
  G4Tubs* photocathodeSolid = new G4Tubs("photocathodeSolid", EJ280Radius-EJ280Thickness, EJ280Radius, pmtThickness*0.5, 0.0, CLHEP::twopi * rad);
  G4LogicalVolume* photocathodeLogicalVolume = new G4LogicalVolume(photocathodeSolid, aluminum, "photocathodeLogicalVolume");
  ...
  //Okay-----------------------------------------------------------------------------------------------------------------------------------------------------------
  std::vector<G4double> refractionIndices(energies.size(), 1.49);//for all energy [1.49,1.49,1.49,1.49...] //****************
  ...
  new G4PVPlacement(0, G4ThreeVector(0.0, 0.0, 0.0), photocathodeLogicalVolume, "photocathode", pmtLogicalVolume, false, 0);
